...
export default function CreateBlogPage() {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "slug") {
      // Allow spaces while typing, but remove other invalid characters
      const cleanedSlug = value
        .replace(/[\/\\]/g, '') // Remove forward slashes and backslashes
  ...
  const handleSlugBlur = (e) => {
    ...
    if (name === "slug") {
      // Format the slug properly when user finishes typing
      ...
      <div className="bg-white rounded-lg p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            {/* Aspect ratio and format note */}
            <p className="text-sm text-gray-500 mt-2 text-center">
             Upload an image with a 16:10 aspect ratio (JPEG, JPG, PNG only)
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {/* <p className="text-sm text-gray-500 mt-1">
                Enter keywords separated by commas (e.g., investment, startup, funding)
              </p> */}
            </div>
          </div>
