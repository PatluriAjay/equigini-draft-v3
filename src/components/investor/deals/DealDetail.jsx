...
export default function DealDetail({ dealSlug }) {
  ...
  // Check if there are any signed documents to show (including local state)
  const hasSignedDocuments = deal && (
    ...
    eoiSubmitted || // Include local EOI state
    ndaSigned // Include local NDA state
  );

  // If current active tab is "Signed Documents" but there are no signed documents, switch to "Executive Summary"
  useEffect(() => {
  ...
  // Function to refresh deal data after signing
  ...
  // Define tab sections after transformedDeal is available
  const tabSections = [
    { label: "Executive Summary", key: "Executive Summary" },
    { label: "Deal Structure & Ticket Size", key: "Deal Structure & Ticket Size" },
    ...(transformedDeal.dealCollateral ? [{ label: "Resources", key: "Resources" }] : []),
    ...(hasSignedDocuments ? [{ label: "Documents", key: "Signed Documents" }] : []),
  ];
            ...
            <div className="card-bordered bg-purple-50 mb-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="card-paragraph">{transformedDeal.teaserDocument.originalname} </p>
                  <span className="card-paragraph text-gray-400">{(transformedDeal.teaserDocument.size / 1024 / 1024).toFixed(1)} MB</span>
                </div>
          ...
          <div className="border-b border-gray-200 mb-6">
            <div className="flex overflow-x-auto scrollbar-hide -mb-px">
              {tabSections.map(tab => (
                <button
                  key={tab.key}
                  className={`mr-4 py-2 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${
                    activeTab === tab.key
          ...
          <div className="mb-8">
            {activeTab === "Executive Summary" && (
              <div>
                ...
                <div className="mt-6">
                  <h4 className="heading-section text-lg">Description</h4>
      ...
      {showEOIModal && (
        <EOIModal
          ...
          onSubmit={async (form) => {
            ...
            // Refresh deal data to get the signed documents
