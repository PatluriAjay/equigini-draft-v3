...
export default function DealsGrid({ 
  ...
  const handleKnowMoreClick = (e, dealSlug) => {
    e.preventDefault();
    e.stopPropagation();
    
    const investorData = getInvestorData();
    if (investorData && !investorData.is_approved) {
      setShowApprovalModal(true);
    } else {
      // Navigate to deal detail page for approved investors
      window.location.href = `/investor/deals/${dealSlug}`;
    }
  };
  ...
  return (
          ...
          >
            ...
            <div className="w-full px-2 pb-4 flex gap-2" onClick={(e) => e.stopPropagation()}>
              <Link href={`/deals/${deal.slug}`} className="flex-1">
                <button
                  onClick={(e) => {
                    handleKnowMoreClick(e, deal.slug);
                  }}
    ...
    <ModalMessage
      ...
      message="Please wait while your access request is reviewed. For assistance, contact admin@equigini.com."
    />
    </>
  );
}
