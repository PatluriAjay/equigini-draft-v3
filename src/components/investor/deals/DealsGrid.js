...
export default function DealsGrid({ maxDeals, layout = "default" }) {
    ...
    <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 ${gridCols} gap-6`}>
      {displayDeals && displayDeals.length > 0 ? (
        displayDeals.map((deal) => (
          <div
            key={deal.id}
            className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-100 flex flex-col"
          >
            {/* Deal Image */}
            ...
            <div className="flex items-center gap-3 mb-2 px-4">
              <div className="card-icon-div ">
                {/* Render icon by sector or icon key */}
