...
export default function AdminSidebar({ collapsed, setCollapsed }) {
  ...
  const links = [{ href: "/admin/eoi", label: "EOI", icon: FiFileText },
  ...
  return (
    <aside
      className={`bg-white backdrop-blur-lg shadow-xl rounded-2xl p-3 pt-6 min-h-[calc(100vh-80px)] flex flex-col justify-between border border-white transition-all duration-300 ${
        collapsed ? "w-16" : "w-60"
      }`}
    >
      <div>
          ...
          <div className="relative">
            <button
              ...
              className={`w-full relative px-3 py-2 rounded-xl transition-all flex items-center gap-3 text-sm
                ${pathname.startsWith("/admin/settings") ? "text-white sidebar-gradient text-base" : "text-gray-700"}
                ${collapsed ? "justify-center px-0" : ""}`}
            >
              <span className={`${iconClass} ${pathname.startsWith("/admin/settings") ? "text-white" : "text-primarycolor"}`}>
                <FiSettings />
              </span>
              {!collapsed && (
                <>
            ...
            </button>            {/* Settings Submenu - Only show when sidebar is expanded */}
                ...
                {[{
                  ...
                  return (
                    <Link
                      ...
                      href={subLink.href}
                      className={`group relative px-3 py-2 rounded-xl transition-all flex items-center gap-3 text-sm
                        ${isSubActive || isSubHovered ? "text-white sidebar-gradient text-base" : "text-gray-700 border-l-0 font-normal"}
                      `}
                      onMouseEnter={() => setHovered(subLink.href)}
                      onMouseLeave={() => setHovered(null)}
                    >
                      <span
                        className={`${iconClass} ${isSubActive || isSubHovered ? "text-white" : "text-primarycolor"} font-semibold group-hover:text-white`}
                      >
                        {subLink.icon && (() => {
                          const Icon = subLink.icon;
                          return <Icon />;
