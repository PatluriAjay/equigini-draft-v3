"use client";
import { MdNotificationsActive, MdSearch, MdMenuOpen } from "react-icons/md";
import { FiSettings, FiLogOut, FiChevronLeft, FiChevronRight, FiMenu } from "react-icons/fi";
  ...
  return (
     <header className="w-full flex items-center justify-between h-[80px] p-5 bg-white ">
        ...
        <div className="flex items-center gap-2">
          {/* Hamburger for mobile, chevrons for desktop */}
          {isMobile ? (
            <button
            ...
            >
              {sidebarOpen ? <MdMenuOpen size={26} /> : <FiMenu size={24} />}
            </button>
          ) : (
            <button
              className="rounded-lg p-2 hover:bg-white/40 transition inline-block"
        ...
        <div className="flex items-center gap-4 relative">
          {/* Nav Links with horizontal slide animation - hidden on mobile */}
          {!isMobile && (
            <nav
